...
mod transmutation {
    ...
    pub struct Transmutation {
        ...
        /// a temp registry to hold 'in transit' assets "escrow"
    ...
    impl Transmutation {
        ...
        pub fn roll(&mut self) -> Result<(), Error> {
            ...
            let seed: [u8; 32] = self.get_seed(acct_id_bytes.try_into().unwrap());
        ...
        pub fn destroy_seed(&mut self) -> Result<(), Error> {
            ...
            if let Some(asset) = self.registry_lookup(caller) {
                ...
                self.pending_swaps.remove(caller);
                return Ok(());
            }
        ...
        pub fn registry_lookup(&self, who: AccountId) -> Option<OpaqueAssetId> {
            if let Some(found_seed) = self.claimed_assets.iter().find(|seed| {
                self.asset_registry
                    .get(seed)
                    .is_some_and(|registry_entry| registry_entry.eq(&who))
            }) {
        ...
        /// create a new swap
        #[ink(message)]
        pub fn try_new_swap(&mut self, who: AccountId, deadline: RoundNumber) -> Result<(), Error> {
            let caller = self.env().caller();
            // make sure caller has an asset
            if let Some(source_asset_id) = self.registry_lookup(caller) {
                // and neither asset is part of a pending swap
                if self.pending_swaps.get(caller).is_none()
                    && self.pending_swaps.get(who).is_none()
                {
                    // get the owner of the target asset id
                    if let Some(target_asset_id) = self.registry_lookup(who) {
                        let merkle_root = Self::calculate_merkle_root(caller, who)?;
                        let swap = Swap {
        ...
        pub fn transmute(&mut self) -> Result<(), Error> {
            ...
            if let Some(merkle_root) = self.pending_swaps.get(caller) {
                if let Some(swap) = self.swaps.get(merkle_root) {
                    // transmutation must only occur at the deadline
                    if let Ok(direction) = self.env().extension().check_time(swap.deadline) {
                        if !TemporalDirection::Present.eq(&direction) {
        ...
        pub fn complete(&mut self, swap_id: Hash) -> Result<(), Error> {
            ...
            if let Some(swap) = self.swaps.take(swap_id) {
                // completion can only happen after the deadline (deadline in past)
                if let Ok(direction) = self.env().extension().check_time(swap.deadline) {
                    if !TemporalDirection::Past.eq(&direction) {
                        return Err(Error::InvalidRoundNumber);
                    }
                    ...
                    if let Some(asset_owner_one) =
                        self.asset_registry.get(swap.asset_id_one.clone())
                    {
                        if let Some(asset_owner_two) =
                            self.asset_registry.get(swap.asset_id_two.clone())
                        {
                            self.asset_registry
                                .insert(swap.asset_id_one.clone(), &asset_owner_two);
                            self.asset_registry
                                .insert(swap.asset_id_two.clone(), &asset_owner_one);
        ...
        /// TODO: leaves should be acct id and asset id in some new struct
        /// a helper function to calculate a merkle root
        pub fn calculate_merkle_root(left: AccountId, right: AccountId) -> Result<Hash, Error> {
    ...
    mod tests {
        ...
        fn can_register_seed() {
            ...
            assert_eq!(
                transmutation.swap_lookup(accounts.alice, accounts.bob),
                Err(Error::SwapDNE)
            );
            ...
            assert_eq!(
                transmutation
                    .asset_registry
                    .get(transmutation.claimed_assets[0].clone())
                    .unwrap(),
                accounts.alice
            );
        }
        ...
        fn test_can_create_new_swap() {
            ...
            if let Err(_) = transmutation.try_new_swap(accounts.bob, deadline) {
                panic!("{:?}", "The test should pass");
            }
            ...
            let merkle_root =
                Transmutation::calculate_merkle_root(accounts.alice, accounts.bob).unwrap();
            assert_eq!(transmutation.swaps.get(merkle_root).unwrap(), expected_swap);
            assert_eq!(
                transmutation
                    .swap_lookup(accounts.alice, accounts.bob)
                    .unwrap(),
